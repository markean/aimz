...
Automatic rerun               Yes (local latents → ``draw``)       No (final mode)
...
* If :meth:`~aimz.ImpactModel.predict` warns about a local-latent posterior, it automatically reruns under ``parallel="draw"``; pass ``parallel="draw"`` explicitly to silence the warning (see :ref:`faq-model-compatibility`).
...
Example: :meth:`~aimz.ImpactModel.predict` with a Local-Latent Rerun
--------------------------------------------------------------------

A common scenario for the rerun warning occurs when the model contains **local latent variables**, which make posterior sample shapes incompatible with data-parallel (observation-sharded) execution; :meth:`~aimz.ImpactModel.predict` then warns and reruns under ``parallel="draw"``.
...
.. jupyter-execute::
    ...
    # Calling `.predict()` warns and reruns under parallel="draw"
    im.predict(X)
