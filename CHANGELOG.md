...
- On GPU/TPU backends, predictive sampling and log-likelihood evaluation now run all draws of a step in one vectorized `jax.vmap` program instead of a sequential `jax.lax.map` loop, matching NumPyro's `Predictive(parallel=True)` execution while keeping streamed memory bounded (the vectorized extent is one `batch_size` chunk per device, so `batch_size` remains the memory dial). CPU behavior is unchanged: the sequential loop keeps temporary memory constant in `batch_size` and scales better across devices.
