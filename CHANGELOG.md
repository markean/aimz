...
- The disk-backed methods now write results with a shared pool of writer threads that serve all return sites, instead of one dedicated thread per return site. In the common case of a single return site — where all disk I/O previously serialized on one thread — the output is now written concurrently across the pool. There is no new API surface: the pool size is chosen automatically from the number of CPUs (capped for safety), bounded by the number of batches and the available host memory (`psutil` is now a required dependency for this), and automatic batch sizing (`batch_size=None`) now also targets enough batches to keep the pool occupied — subject to a floor on per-batch output bytes so small workloads stay whole — in addition to the existing per-batch memory ceiling, so large outputs that previously streamed as one batch (or a few ~100 MB batches) are written as several concurrent chunks and read back with a correspondingly finer Dask chunking. Results are unchanged: writes are order-independent and target disjoint, chunk-aligned Zarr chunks; an explicit `batch_size` is honored exactly as before; models with an observation-aligned posterior keep their previous single-batch behavior on one device; and the append fallback for length-less data loaders remains single-consumer to preserve ordering.
