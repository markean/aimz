...
- The disk-backed methods now write results with a shared pool of writer threads that serve all return sites, instead of one dedicated thread per return site. In the common case of a single return site — where all disk I/O previously serialized on one thread — the output is now written concurrently across the pool. The pool size is chosen automatically from the number of CPUs (capped for safety), bounded by the number of batches and the available host memory, so there is no new API surface. Writes are order-independent and target disjoint, chunk-aligned Zarr chunks, so results are unchanged; the append fallback used for length-less data loaders remains single-consumer to preserve ordering.
- Automatic batch sizing in the disk-backed methods (`batch_size=None`) now also targets enough batches to keep the writer pool occupied, subject to a floor on per-batch output bytes so small workloads stay whole, in addition to the existing per-batch memory ceiling. Large outputs that previously streamed as one batch (or a few ~100 MB batches) are now written as several concurrent chunks, and read back with a correspondingly finer Dask chunking. Results are unchanged; an explicit `batch_size` is honored exactly as before. Models with an observation-aligned posterior keep their previous single-batch behavior on one device rather than falling back to draw-parallel streaming.
- `psutil` is now a required dependency (previously optional). It sizes the writer pool and its queue against available host memory; the blind fallback used when it was missing could severely throttle the writer pool.
