...
- The methods that accept an `intervention` mapping, including lazily generated scenarios in {meth}`~aimz.ImpactModel.estimate_effect`, now raise a `ValueError` when it names a site that is not a sample site of the kernel, such as a misspelled name or a deterministic site, instead of silently returning the output without that intervention ([#329](https://github.com/markean/aimz/issues/329)).
...
- The disk-backed methods now write results with a shared pool of writer threads that serve all return sites, instead of one dedicated thread per return site. There is no new API surface: the pool size is chosen automatically from the number of CPUs, bounded by the number of batches and the available host memory (`psutil` is now a required dependency for this), and automatic batch sizing (`batch_size=None`) now also targets enough batches to keep the pool occupied. An explicit `batch_size` is honored as before; models with an observation-aligned posterior keep their previous single-batch behavior on one device; and the append fallback for length-less data loaders remains single-consumer to preserve ordering ([#287](https://github.com/markean/aimz/issues/287)).
- Requesting `return_sites` not seen in any kernel trace now emits a `UserWarning` and passes the names through instead of raising an error, allowing sites that a kernel defines only at sampling time. Fitting also no longer discards sites discovered by an earlier trace: the fitting re-trace merges them, so sampling-branch sites remain known and requestable without a warning. As a result, dual-branch kernels with a custom likelihood can now run prior predictive sampling before fitting ([#289](https://github.com/markean/aimz/issues/289)).
- The "`batch_size` is not divisible by the number of devices" warning is no longer emitted when `batch_size=None` and the size was resolved automatically ([#292](https://github.com/markean/aimz/issues/292)).
