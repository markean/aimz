...
- {meth}`~aimz.ImpactModel.predict`, {meth}`~aimz.ImpactModel.log_likelihood`, {meth}`~aimz.ImpactModel.sample_prior_predictive`, and {meth}`~aimz.ImpactModel.sample_posterior_predictive` now accept a `parallel` argument selecting the multi-device sharding strategy. The default `"data"` shards the observation axis of the input across devices and replicates the posterior (the previous behavior). The new `"draw"` shards the drawn samples across devices in chunks of `batch_size` draws while holding the whole input resident — a multi-device, memory-bounded analog of {external:class}`numpyro.infer.Predictive` with `parallel=True`. Because the observation axis is never split, `"draw"` streams any model — including ones with per-observation (local) latent variables (a sample site whose posterior is shaped `(num_samples, n_obs)`) — with no fallback and no plate requirement. In `"draw"` mode `batch_size` is the number of draws per chunk and `X` must be an in-memory array. {meth}`~aimz.ImpactModel.estimate_effect` inherits the option through the prediction arguments it forwards. This has no sharding effect on a single device. Invalid `parallel` values raise a `ValueError`.
- {meth}`~aimz.ImpactModel.sample_prior_predictive` under `parallel="draw"` draws a complete fresh prior sample per chunk against the whole input, so per-observation latent variables are sampled correctly (request them via `return_sites`). Under `parallel="data"` it keeps the previous observation-streaming behavior for models with only global latents.

### Changed

- For a model with a local (observation-aligned) latent variable, {meth}`~aimz.ImpactModel.predict` under the default `parallel="data"` now warns and reruns under `parallel="draw"` instead of falling back to the in-memory {meth}`~aimz.ImpactModel.predict_on_batch`, keeping results streamed to disk and memory-bounded. {meth}`~aimz.ImpactModel.log_likelihood`, which previously had no fallback for such models, behaves the same way. Passing `parallel="draw"` explicitly silences the warning.
