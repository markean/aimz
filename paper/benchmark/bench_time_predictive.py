"""Benchmark wall-clock time for NumPyro `Predictive`."""
...
def generate_synthetic_data(rng_key: Array, n: int, p: int) -> tuple[Array, Array]:
    ...
    Data is generated as JAX arrays on CPU because NumPyro's `Predictive` requires JAX
    arrays. The arrays are later moved to the compute device via `device_put`.
    """
...
def main() -> None:
    """Benchmark NumPyro `Predictive` wall-clock time."""
    parser = argparse.ArgumentParser(
        description="Benchmark NumPyro Predictive runtime.",
    )
    ...
    logger.info("Running Predictive...")
    rng_key, rng_subkey = random.split(rng_key)
    start = time.perf_counter()
    out = predictive(rng_subkey, X)
    block_until_ready(out)
    elapsed = time.perf_counter() - start
    logger.info("Elapsed: %.6f s", elapsed)
