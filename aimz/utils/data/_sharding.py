...
def _create_sharded_sampler(
    ...
    n_kwargs_extra: int,
    *,
    parallel: Literal["data", "draw"] = "data",
) -> Callable:
    ...
    Args:
        ...
        n_kwargs_extra: The number of extra keyword arguments that are not array-like
            (not sharded).
        parallel: ``"data"`` (default) shards the observation axis of ``X`` and the
    ...
    Returns:
        A sharded function that takes the following arguments:
            ...
            - num_samples (int): The number of samples to draw (per-device under
                ``"draw"`` sharding).
            - rng_key: A scalar PRNG key under ``"data"`` sharding, or a pre-split
    ...
    rng_spec = PartitionSpec(axis) if draws else PartitionSpec()
    samples_spec = PartitionSpec(axis) if draws else PartitionSpec()
    x_spec = PartitionSpec() if draws else PartitionSpec(axis)
    kw_spec = PartitionSpec() if draws else PartitionSpec(axis)
    out_spec = PartitionSpec(axis, None) if draws else PartitionSpec(None, axis)
...
def _create_sharded_log_likelihood(
    ...
    *,
    ...
    Args:
        ...
        n_kwargs_extra: The number of extra keyword arguments that are not array-like
            (not sharded).
        parallel: ``"data"`` (default) shards the observation axis of ``X``/``y`` and
    ...
    (axis,) = mesh.axis_names
    samples_spec = PartitionSpec(axis) if draws else PartitionSpec()
    xy_spec = PartitionSpec() if draws else PartitionSpec(axis)
    kw_spec = PartitionSpec() if draws else PartitionSpec(axis)
    out_spec = PartitionSpec(axis, None) if draws else PartitionSpec(None, axis)
...
def _prepare_draw_chunk(
    ...
    multiple of ``num_devices`` (so it splits evenly under sample-parallel sharding),
